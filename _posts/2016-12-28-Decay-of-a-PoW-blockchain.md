...
$$\text{sha256}(\text{sha256}(\text{version} | \text{hash}\_\text{prev} | \text{merkle}\_\text{root}\_\text{hash} | \text{timestamp} | \text{bits} | \text{nonce})) < \text{target},$$
...
$$\nonumber s = \frac{\text{current target}}{2^{256}}$$
...
$\tau$ is a discrete random variable. In fact, cryptoplatforms use [Unix time](https://en.wikipedia.org/wiki/Unix_time) for timestamps, so time is written to chain in seconds and milliseconds are neglected. 


Unlike the variables $\xi$ and $\tau$, $h$ is a continuous and non-random one. Original equipment manufacturers (OEM) are not able to determine a whole number of hashes generated by their hardware. In addition, the equipment deterioration can slow down the process of calculations (the equipment will compute a few billion hashes not in one second, but in one second and a little bit more). In this case a number of hashes per second is definitely fractional.
...
В общем случае задача сводится ко взятию $(N-2)$-кратного интеграла $\iint\limits_D dx_1 dx_2 ... dx_{N-2}$ по области $$D = {x_1, x_2, ..., x_{N-2} \mid \:  x_1 < p,\: x_2 < p,\: ...,\: x_{N-2} < p,\: x_1 + x_2 + ... + x_{N-2} > 100-2p  	} .$$
