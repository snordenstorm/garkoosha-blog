...
What I love mathematics for is how easy questions can slowly unravel into very complicated solutions. I love locally trivial objects that are nontrivial globally.
...
The benefit of using the random variable $\xi$ is that its probability distribution is known. In fact, the probability for $\xi = 1$ is equal to $s$. The probability for $\xi = 2$ is equal to $s(1−s)$. The probability for $\xi = 3$ is equal to $s(1 − s)^2$, and so on. It's pretty clear why: for any given $\xi$, all hashes except the last do not solve block (giving multipliers $(1-s)$ for each attempt), and the last one does (giving multiplier $\xi$).
...
$$\nonumber M\xi = 1 \cdot s + 2 \cdot s(1-s) + 3 \cdot s(1-s)^2 + ... = s (1 + 2(1-s) + 3 (1-s)^2 + 4 (1-s)^3 + ...) = $$ 
...
At this point, we would like to remind what affects the probability $s$. Solve a block is essentially solving the following inequality:

$$\nonumber \text{sha256}(\text{sha256}(\text{version} | \text{hash}\_\text{prev} | \text{merkle}\_\text{root}\_\text{hash} | \text{timestamp} | \text{bits} | \text{nonce})) < \text{target},$$

Clearly, the higher _target_ is, the higher is the chance. Since _target_ is 256-bit number, assuming double-sha256 provides us with homogeneous distribution, 

$$\nonumber s = \frac{\text{target}}{2^{256}}$$

Mining _difficulty_ and _target_ are interrelated: 

$$\nonumber \text{difficulty} = \frac{C}{\text{target}}$$ 

The creator of Bitcoin has chosen the following value for 

$$\nonumber C = \text{0xffff}\underbrace{00...0}\_{\text{51 times}} = 15 \cdot 16^{55} + 15 \cdot 16^{54} + 15 \cdot 16^{53} + 15 \cdot 16^{52} = $$ 

$$\nonumber = 16 \cdot 2^{220} + 16 \cdot 2^{216} + 16 \cdot 2^{212} + 16 \cdot 2^{208} - 2^{220} - 2^{216} - 2^{212} - 2^{208} =  2^{224} - 2^{208} \approx 2^{224}$$ 

between target and difficulty, the result will be: 

$$ \nonumber \label{sdiff} s = \frac{\text{current target}}{2^{256}} =\frac{2^{224}/\text{current difficulty}}{2^{256}} = \frac{1}{2^{32} \cdot \text{current difficulty}} $$

It is clear that the probability to solve a block depends solely on difficulty value. Remember that we had previously made the assumption that, from the moment when the network split into segments till the moment when a node determines a segment as major or minor, the _difficulty_ value was not adjusted. With this assumption $s$ is a known constant.


#### Random variable $\tau$ and its probability distribution


Time to solve a block is definitely linked to a number of hashes that have to be calculated. Let $T_i$ be an amount of time to solve a block and $x_i$ be a number of hashes calculated for that. Let $h$ (hash/s) be hash rate of the entire segment. Therefore, $$x_i = h T_i.$$ It is true that the previously considered random variables $\tau$ and $\xi$ are interrelated: $$\label{xitau}\xi = h \tau.$$

$\tau$ is a discrete random variable. In fact, cryptoplatforms use [Unix time](https://en.wikipedia.org/wiki/Unix_time) for timestamps, so times are written to chains in seconds and milliseconds are neglected. 


Unlike the variables $\xi$ and $\tau$, $h$ is a non-integer and non-random one. Original equipment manufacturers (OEM) do not produce devices with integer hash/s hashrate, and, clearly, the equipment deterioration can slow down the process of calculations (the equipment will compute a few billion hashes not in one second, but in one second and a little bit more). In this case a number of hashes per second is definitely fractional.
...
$$\nonumber P(\tau = 1) = \sum\limits_{i=0}^{h-1} s(1-s)^i = s \frac{1 - (1-s)^h}{1 - (1-s)} = 1 - (1-s)^h$$
...
$$\nonumber P(\tau = 2) = \sum\limits_{i=h}^{2h-1} s(1-s)^i = (1-s)^h P(0) = (1-s)^h - (1-s)^{2h}$$
...
$$\nonumber P(\tau = 3) = \sum\limits_{i=2h}^{3h-1} s(1-s)^i = (1-s)^h P(1) = (1-s)^{2h} P(0) = (1-s)^{2h} - (1-s)^{3h}$$
...
$$\nonumber M\tau = 1 \cdot \left( 1 - (1-s)^h \right) + 2 \cdot \left( (1-s)^h - (1-s)^{2h} \right) + 3 \cdot \left( (1-s)^{2h} - (1-s)^{3h} \right) + ...  =$$ 

$$= 1 + (1-s)^h + (1-s)^{2h} + (1-s)^{3h} + ... = \frac{1}{1 - (1-s)^h} = \frac{1}{1 - x}$$

(Here we introduced the notation $(1-s)^h \equiv x$.)

$$\nonumber M\tau^2 = 1 \cdot \left( 1 - (1-s)^h \right) + 4 \cdot \left( (1-s)^h - (1-s)^{2h} \right) + 9 \cdot \left( (1-s)^{2h} - (1-s)^{3h} \right) + ...  =$$ 

$$\nonumber = 1 + 3 (1-s)^h + 5 (1-s)^{2h} + 7 (1-s)^{3h} + ... = \sum\limits_{j=0}^{\infty} (2j+1) \left( (1-s)^h \right)^j $$ 



$$\nonumber = \sum\limits_{j=0}^{\infty} (2j+1) x^j = 2\sum\limits_{j=0}^{\infty} j x^j + \sum\limits_{j=0}^{\infty}  x^j = 2x\sum\limits_{j=0}^{\infty} j x^{j-1} + \frac{1}{1-x} = 2x\sum\limits_{j=0}^{\infty} \left(x^j \right)' + \frac{1}{1-x} = $$ 

$$\nonumber = 2x  \left(\sum\limits_{j=0}^{\infty} x^j \right)' + \frac{1}{1-x} = 2x  \left( \frac{1}{1-x} \right)' + \frac{1}{1-x} = 2x \cdot\frac{1}{(1-x)^2} + \frac{1}{1-x} = \frac{2x + (1-x)}{(1-x)^2} = \frac{1+x}{(1-x)^2}$$
...
In order to compute (functions having this property are called _central statistics_): 
...
$$\nonumber M\eta = M\left( \frac{\sum\limits_{i=1}^n T_i - n M\tau}{\sqrt{n D\tau}} \right) = M\left( \frac{\sum\limits_{i=1}^n T_i }{\sqrt{n D\tau}} \right) - M\left( \frac{n M\tau}{\sqrt{n D\tau}} \right) \xrightarrow{n\to\infty}  \frac{n \cdot M\tau}{\sqrt{n D\tau}} - \frac{n M\tau}{\sqrt{n D\tau}}   = 0$$

$$\nonumber D\eta = D\left( \frac{\sum\limits_{i=1}^n T_i - n M\tau}{\sqrt{n D\tau}} \right) = \frac{1}{nD\tau} D\left(\sum\limits_{i=1}^n T_i - n M\tau \right) = \frac{1}{nD\tau} D\left(\sum\limits_{i=1}^n T_i \right) \xrightarrow{n\to\infty} \frac{1}{nD\tau} \cdot (n \cdot D\tau) = 1.$$
...
$$\nonumber L = \lim\limits_{\varepsilon \to 0} \frac{\frac{1}{2}  \text{erf}\left( \left( (\overline{T} - 1) - \overline{T} (1-s)^{h_0 + \varepsilon} \right) \sqrt{\frac{n\overline{T}}{2(\overline{T}-1)}}  \right) - \frac{1}{2} \text{erf}\left( \left( (\overline{T} - 1) - \overline{T} (1-s)^{h_0 - \varepsilon} \right) \sqrt{\frac{n\overline{T}}{2(\overline{T}-1)}} \right)}{2\varepsilon} = $$ 

$$\nonumber =  \lim\limits_{\varepsilon \to 0} \frac{\text{erf} \left(p - q (1-s)^{\varepsilon} \right) - \text{erf} \left(p - q (1-s)^{-\varepsilon} \right)}{4\varepsilon},$$ 
...
$$\nonumber\text{erf} \left(p - q (1-s)^{\varepsilon} \right) - \text{erf} \left(p - q (1-s)^{-\varepsilon} \right) = \frac{2}{\sqrt{\pi}} \sum\limits_{n=0}^{\infty} \frac{(-1)^n}{n! (2n+1)} \left[ \left( p - q (1-s)^{\varepsilon} \right)^{2n+1}  - \left( p - q (1-s)^{-\varepsilon} \right)^{2n+1} \right] = $$ 

$$\nonumber = \frac{2}{\sqrt{\pi}} \sum\limits_{n=0}^{\infty} \frac{(-1)^n}{n! (2n+1)} \left[ \left( p - q - \varepsilon q \ln(1-s) + o(\varepsilon) \right)^{2n+1}  - \left( p - q + \varepsilon q \ln(1-s) + o(\varepsilon) \right)^{2n+1} \right] = $$ 

$$\nonumber = \frac{2}{\sqrt{\pi}} \sum\limits_{n=0}^{\infty} \frac{(-1)^n}{n! (2n+1)} \left[ (2n+1) (p-q)^{2n} (-\varepsilon q \ln(1-s)) - (2n+1) (p-q)^{2n} \varepsilon q \ln(1-s) + o(\varepsilon) \right] = $$

$$\nonumber = \frac{-4\varepsilon q \ln(1-s)}{\sqrt{\pi}} \sum\limits_{n=0}^{\infty} \frac{(-1)^n}{n!} (p-q)^{2n}  + o(\varepsilon)= \frac{-4\varepsilon q \ln(1-s)}{\sqrt{\pi}} \sum\limits_{n=0}^{\infty} \frac{(-1)^n ((p-q)^2)^n}{n!}  + o(\varepsilon) = $$ 

$$\nonumber = \frac{-4\varepsilon q \ln(1-s)}{\sqrt{\pi}} \sum\limits_{n=0}^{\infty} \frac{(- (p-q)^2)^n}{n!}  + o(\varepsilon) =  \frac{-4\varepsilon q \ln(1-s)}{\sqrt{\pi}} \exp(- (p-q)^2) + o(\varepsilon)$$

We finish calculating the limit:

$$\nonumber L = \lim\limits_{\varepsilon \to 0} \frac{\frac{-4\varepsilon q \ln(1-s)}{\sqrt{\pi}} \exp(- (p-q)^2) + o(\varepsilon)}{4\varepsilon} =  \frac{-q \ln(1-s)}{\sqrt{\pi}} \exp(- (p-q)^2).$$ 

Substituting $p$ and $q$ with their values, we obtain 

$$ \nonumber L =  \frac{-\ln(1-s)}{\sqrt{\pi}} \cdot \overline{T} (1-s)^{h_0} \sqrt{\frac{n\overline{T}}{2(\overline{T}-1)}}  \cdot \exp\left(- \frac{n\overline{T}}{2(\overline{T}-1)} (\overline{T} - 1 - \overline{T} (1-s)^{h_0})^2 \right)  $$ 
...
$$ \nonumber p = \frac{h}{H} \cdot 100 $$ 

(By definition, $0 < p < 100$.)

We have 

$$\nonumber P(h_0 - \varepsilon < h < h_0 + \varepsilon) = P\left(h_0 - \varepsilon < \frac{pH}{100} < h_0 + \varepsilon\right) = $$ 

$$\nonumber = P\left( \frac{100}{H} h_0 - \frac{100}{H} \varepsilon < p < \frac{100}{H} h_0 + \frac{100}{H} \varepsilon \right) = P(p_0 - \epsilon < p < p_0 + \epsilon)$$ 

(Here we introduced new variables $p_0 = \frac{100}{H} h_0$, $\epsilon = \frac{100}{H} \varepsilon$.) Then

$$\nonumber \lim\limits_{\epsilon \to 0} \frac{P(p_0 - \epsilon < p < p_0 + \epsilon)}{2\epsilon} = \lim\limits_{\epsilon \to 0} \frac{P(h_0 - \varepsilon < h < h_0 + \varepsilon)}{2\epsilon} =$$ 

$$\nonumber = \lim\limits_{\epsilon \to 0} \frac{P(h_0 - \varepsilon < h < h_0 + \varepsilon)}{2\varepsilon} \frac{H}{100} = \lim\limits_{\varepsilon \to 0} \frac{P(h_0 - \varepsilon < h < h_0 + \varepsilon)}{2\varepsilon} \frac{H}{100} = $$ 

$$\nonumber =  \frac{-\ln(1-s)}{\sqrt{\pi}} \cdot  (1-s)^{h_0} \sqrt{\frac{n\overline{T}^3}{2(\overline{T}-1)}}  \cdot \exp\left(- \frac{n\overline{T}^3}{2(\overline{T}-1)} \left(\frac{\overline{T} - 1}{\overline{T}} - (1-s)^{h_0}\right)^2 \right) \cdot  \frac{H}{100} = $$ 

$$\nonumber =  \frac{-\ln(1-s)}{\sqrt{\pi}} \cdot  (1-s)^{\frac{p_0 H}{100}} \sqrt{\frac{n\overline{T}^3}{2(\overline{T}-1)}}  \cdot \exp\left(- \frac{n\overline{T}^3}{2(\overline{T}-1)} \left(\frac{\overline{T} - 1}{\overline{T}} - (1-s)^{\frac{p_0 H}{100}}\right)^2 \right) \cdot  \frac{H}{100}$$
...
Now Taylor series enter the game: since $sH \ll 1$ (proof: $sHT = 1 \to sH = 1/T \to sH \ll 1$), $(1-s)^H \approx 1 - sH$ holds. So 

$$1 - sH < 1 - \frac{1}{\overline{T}} \qquad \longrightarrow \qquad (1-s)^H < \frac{\overline{T}-1}{\overline{T}} \qquad \longrightarrow \qquad (1-s)^H < \gamma$$
...
$$\nonumber p_{\text{maj}} = \int\limits_0^{100} \rho(p) \cdot V(p) \: dp = \int\limits_0^{100/N} \rho(p) \cdot 0 \: dp + \int\limits_{100/N}^{50} \rho(p) \cdot \frac{3p-100}{100-p} \: dp + \int\limits_{50}^{100} \rho(p) \cdot 1 \: dp $$
...
In general case of arbitrary $N$, the problem degenerates to calculating $(N-2)$-tuple integral $\iint\limits_D dx_1 dx_2 ... dx_{N-2}$ by area $$D = \{ x_1, x_2, ..., x_{N-2} \mid \:  x_1 < p,\: x_2 < p,\: ...,\: x_{N-2} < p,\: x_1 + x_2 + ... + x_{N-2} > 100-2p  	\} .$$
...
the solution is fortunately [known](http://math.stackexchange.com/questions/454583/volume-of-cube-section-above-intersection-with-plane): 

$$\label{unit} V = 1 - \sum\limits_{i=0}^{\text{floor} (\alpha)} (-1)^i C^i_{N-2} \frac{(\alpha - i)^{N-2}}{(N-2)!} $$
...
$$ \label{V} \boxed{V(p) = \left\{ \begin{array}{ll}
        0, \hskip 3 pt & \text{if} \: p < 100/N \\
        \left(\frac{p}{100-p}\right)^{N-2} \left( 1 - \sum\limits_{i=0}^{\text{floor} (100/p) - 2} (-1)^i C^i_{N-2} \frac{(\frac{100-2p}{p} - i)^{N-2}}{(N-2)!} \right), \hskip 3 pt & \text{if} \: 100/N \leqslant p \leqslant 50 \\
        1, \hskip 3 pt & \text{if} \: p > 50.
...
$$\nonumber \rho(p) = \frac{-\ln(1-s)}{\sqrt{\pi}} \cdot \overline{T} (1-s)^{\frac{pH}{100}} \sqrt{\frac{n\overline{T}}{2(\overline{T}-1)}}  \cdot \exp\left(- \frac{n\overline{T}}{2(\overline{T}-1)} \left(\overline{T} - 1 - \overline{T} (1-s)^{\frac{pH}{100}} \right)^2 \right) = $$ 

$$\nonumber = \frac{-\ln(1-s)}{\sqrt{\pi}} \cdot \overline{T} (1-s)^{\frac{pH}{100}} \sqrt{\frac{n\overline{T}}{2(\overline{T}-1)}}  \cdot \exp\left(- \frac{n\overline{T}^3}{2(\overline{T}-1)} \left(\frac{\overline{T} - 1}{\overline{T}} - (1-s)^{\frac{pH}{100}}\right)^2 \right)  $$
...
$$\nonumber (1-s)^{\frac{pH}{100}} \equiv y, \qquad \frac{n\overline{T}^3}{2(\overline{T}-1)} \equiv \beta, \qquad \frac{\overline{T}-1}{\overline{T}} \equiv \gamma$$ 

we have 

$$\nonumber \rho(y) =  \frac{-\ln(1-s)}{\sqrt{\pi}} \cdot y \sqrt{\beta}  \cdot \exp\left(- \beta (\gamma - y)^2 \right), \hskip 20 pt \beta > 120.$$
...
$$\nonumber \delta\left(b - a^{cp} \right) = \frac{\delta\left( p - \frac{1}{c} \log_a b \right)}{\left| - c a^{c \frac{1}{c} \log_a b} \ln a \right|} = \frac{\delta\left( p - \frac{1}{c} \log_a b \right)}{\left| - c b \ln a \right|} = \frac{1}{-\ln a} \cdot \frac{\delta\left( p - \frac{1}{c} \log_a b \right)}{bc} $$
...
$$\nonumber \rho (p) = -\ln(1-s) \cdot (1-s)^{\frac{pH}{100}} \cdot \frac{1}{-\ln (1-s)} \cdot \frac{\delta\left( p - \frac{100}{H} \log_{1-s} \gamma \right)}{H \gamma/100} = (1-s)^{\frac{p H}{100}} \frac{100}{H \gamma} \delta\left( p - \frac{100}{H} \log_{1-s} \gamma \right)$$

Why did we do all that? The answer is, there is no function nicer to integrate than Dirac $\delta$-function: 

$$\nonumber \int\limits_{-\infty}^{\infty} f(x) \delta(x - x_0) dx = f(x_0)$$ 

The answer to an original problem is integral of $\rho(p) V(p)$. The possibility to rephrase one of the multipliers of $\rho(p)$ as a $\delta$-function means that, instead of calculating the integral, one just has to calculate the value of $V(p)$ in some point. Namely,

$$\nonumber p_{\text{maj}} = \int\limits_0^{100} \rho(p) V(p) dp = \int\limits_0^{100} (1-s)^{\frac{p H}{100}} \frac{100}{H \gamma} \delta\left( p - \frac{100}{H} \log_{1-s} \gamma \right) V(p) dp = $$ 

$$\nonumber = (1-s)^{\log_{1-s} \gamma} \frac{100}{H \gamma}  V\left(\frac{100}{H} \log_{1-s} \gamma\right) = \frac{100}{H}  V\left(\frac{100}{H} \log_{1-s} \gamma\right)$$
